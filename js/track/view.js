...
define(['utils/dom'], function(D$) {
	...
	TrackView.prototype.listenForModel = function() {
		this.model.on('change', function(attr, value, model) {
	...
	TrackView.prototype.renderButtons = function(parentElement) {
			var button1, button2;
			button1 = D$.create('button', { parent: parentElement, className: 'btn btn-xs btn-danger' });
			D$.create('span', { parent: button1, className: 'glyphicon glyphicon-remove'});

			button2 = D$.create('button', { parent: parentElement, className: 'btn btn-xs btn-primary' });
